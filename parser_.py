...
class Parser:
    ...
    def _if_stmt(self):
        ...
        # Only allocate false_lbl now; end_lbl is only needed when else exists
        ...
        if self._match("Keyword", "else"):
            end_lbl = self._new_label()          # allocated only when needed
    ...
    def _block_statements(self):
        """Parse statements inside a { } block with proper scope management.
        Variables declared inside the block are removed when the block exits,
        so they cannot be referenced from outer scopes.
        """
        names_before = {s.name for s in self.symbol_table}
        self._statements()
        # Remove any symbols that were declared inside this block
        self.symbol_table = [
            s for s in self.symbol_table if s.name in names_before
        ]
...
if __name__ == "__main__":
    ...
    p = Parser(tokens)
    p.parse()
    p.write_symbol_table("symbol_table.txt")
    p.write_tac("tac.txt")
    p.write_parser_output("parser_output.txt")
    p.write_errors("errors.txt")
    print("Parser done.")
